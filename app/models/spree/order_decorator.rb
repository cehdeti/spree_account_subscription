Spree::Order.class_eval do
  ...
  # We have to do this here because sometimes the order total is 0 and the
  # payment state transitions don't happen.
  #
  # TODO: It would be nice to create all of the order subscriptions here, but
  # since we need to wait until the order is paid (in the case of purchase
  # orders, for example) and there is no easy way to hook into payment state
  # from the order itself, we have to do it in both places.
  state_machine.after_transition to: :complete, do: :create_subscriptions

  def create_subscriptions
    return unless total.zero? || paid?

    self.user = Spree::User.find_by(email: email) unless user

    line_items.select { |i| i.variant.subscribable? }.each do |line_item|
      if Spree::AccountSubscription.where(order: self, product: line_item.variant.product).exists?
        next
      end
